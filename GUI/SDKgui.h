...
#define GUI_SIZE_SMALL				13
#define GUI_SIZE_MIDDLE				16
#define GUI_SIZE_LARGE				25
#define GUI_TEXTURE_DEFAULT			0

#define GUI_NORMAL				0
#define GUI_ROLLOVER			1
#define GUI_PRESSED				2
#define GUI_DISABLE				3
#define GUI_CHECKED				4
#define GUI_CHECKED_ROLLOVER	5
#define GUI_CHECKED_PRESSED		6
#define GUI_CHECKED_DISABLE		7

#define GUI_FOCUS				8
#define GUI_CHECKED_FOCUS		9
#define GUI_HIDDEN				10
...
// ID of Progress
#define ID_PROGRESS_BOX				0
#define ID_PROGRESS_CHUNK			9

// ID of CheckBox and RadioButton
#define ID_CHECKBOX_LARGE			0
#define ID_CHECKBOX_MIDDLE			8
#define ID_CHECKBOX_SMALL			16

// ID of Slider
#define ID_SLIDER_BUTTON			0
#define ID_SLIDER_TRACK				36

// ID of Combobox
#define ID_COMBO_BUTTON				0
#define ID_COMBO_ARROW				36
#define ID_COMBO_BOX				40

// ID of ScrollBar
#define ID_THUMB_BUTTON_HORIZONTAL	0
#define ID_THUMB_BUTTON_VERTICAL	36
#define ID_ARROW_BUTTON_TOP			72
#define ID_ARROW_BUTTON_BOTTOM		108
#define ID_ARROW_BUTTON_LEFT		144
#define ID_ARROW_BUTTON_RIGHT		180
#define ID_TRACK_TOP				216
#define ID_TRACK_BOTTOM				220
#define ID_TRACK_LEFT				224
#define ID_TRACK_RIGHT				228
#define ID_ARROW_SMALL_TOP			232
#define ID_ARROW_SMALL_BOTTOM		236
#define ID_ARROW_SMALL_LEFT			240
#define ID_ARROW_SMALL_RIGHT		244
#define ID_ARROW_LARGE_TOP			248
#define ID_ARROW_LARGE_BOTTOM		252
#define ID_ARROW_LARGE_LEFT			256
#define ID_ARROW_LARGE_RIGHT		260
#define ID_GRIPPER_HORIZONTAL		264
#define ID_GRIPPER_VERTICAL			268
...
{ 
	...
	DXUT_CONTROL_GROUPBOX,
	DXUT_CONTROL_PROGRESS,
};
...
enum DXUT_CONTROL_STATE
{
	...
	DXUT_STATE_DISABLED,
	DXUT_STATE_CHECKED,
	DXUT_STATE_CHECKED_MOUSEOVER,
	DXUT_STATE_CHECKED_PRESSED,
	DXUT_STATE_CHECKED_DISABLE,
    DXUT_STATE_HIDDEN,
    DXUT_STATE_FOCUS,
};

#define MAX_CONTROL_STATES 10
...
public:
    ...
    void SetFont( UINT iFont, D3DCOLOR defaultFontColor = D3DCOLOR_ARGB(255, 255, 255, 255), DWORD dwTextFormat = DT_NOCLIP | DT_CENTER | DT_VCENTER );
	void SetBorder( D3DCOLOR bColor = D3DCOLOR_ARGB(255, 127, 157, 185),UINT b = 1){   BorderColor.Init( bColor ); iBorder = b;}
	void SetFillColor( D3DCOLOR fColor = D3DCOLOR_ARGB(255, 255, 255, 255)){  FillColor.Init(fColor); }
	void SetImageBgType(bool border){ bBorderFill = border ;}
	void SetImageCount(int count){ iImageCount = count ;}
	bool isImageBgType(){ return bBorderFill; }
    void Refresh();
    ...
    DXUTBlendColor BorderColor;
	DXUTBlendColor FillColor;
};
...
public:
    CDXUTDialog();
    ~CDXUTDialog();
	// get 9 clips of the element;
	void ImageRect(DXUT_CONTROL_TYPE id, CDXUTElement &e,RECT &rc,RECT &sm, int iStat=0 );
	void ImageRect(DXUT_CONTROL_TYPE id, CDXUTElement &e,RECT &rc, int iStat=0 );
    ...
    HRESULT AddCheckBox( int ID, LPCWSTR strText, int x, int y, int height, bool bChecked=false, UINT nHotkey=0, bool bIsDefault=false, CDXUTCheckBox** ppCreated=NULL );
    HRESULT AddRadioButton( int ID, UINT nButtonGroup, LPCWSTR strText, int x, int y, int height, bool bChecked=false, UINT nHotkey=0, bool bIsDefault=false, CDXUTRadioButton** ppCreated=NULL );
    ...
    HRESULT AddSlider( int ID, int x, int y, int width, int height,int buttonWidth,int trackHeight, int min=0, int max=100, int value=50, bool bIsDefault=false, CDXUTSlider** ppCreated=NULL );
	...
	HRESULT AddGroupBox( int ID, LPCWSTR strText, int x, int y, int width, int height, bool bIsDefault=false, CDXUTGroupBox** ppCreated=NULL );
	HRESULT AddProgress( int ID, int x, int y, int width, int height,int value=0, int max=100, bool bIsHorizontal=true, bool bIsDefault=false, CDXUTProgress** ppCreated=NULL );
    ...
    CDXUTGroupBox*     GetGroupBox( int ID ) { return (CDXUTGroupBox*) GetControl( ID, DXUT_CONTROL_GROUPBOX ); }
    CDXUTProgress*     GetProgress( int ID ) { return (CDXUTProgress*) GetControl( ID, DXUT_CONTROL_PROGRESS ); }
...
public:
	...
	void DrawResize( RECT &rc, int iState,int ID, float fElapsedTime );
	void DrawTile( RECT &rc, int iState,int ID, float fElapsedTime, int nCount=1, bool bIsHorizontal=true );
	void DrawBorder( RECT &rcWindow,int iState, float fElapsedTime );
	void DrawStretch( int iState,int ID, RECT* rc, float fElapsedTime, int i=0);
	void SetStretch( int ID, RECT &rc, RECT* rcWindow,int i=0 );
...
public:
	...
	//void SetStretch( int ID, RECT &rc,int i=0 );
	//void DrawStretch( int iState, int ID, float fElapsedTime,int i=0 );
protected:
    bool m_bPressed;
	RECT m_rcStretch[9];
};
...
protected:
    void SetCheckedInternal( bool bChecked, bool bClearGroup, bool bFromInput );
    UINT m_nButtonGroup;
};
...
{
public:
    CDXUTScrollBar( CDXUTDialog *pDialog = NULL , bool IsHorizontal = false );
...
protected:
	...
	bool m_bDrag;
	bool m_bArrowMinPressed;
	bool m_bArrowMaxPressed;
	bool m_bTrackMinPressed;
	bool m_bTrackMaxPressed;
	bool m_bArrowMinMouseOver;
	bool m_bArrowMaxMouseOver;
	bool m_bTrackMinMouseOver;
	bool m_bTrackMaxMouseOver;
...
class CDXUTListBox : public CDXUTButton
{
public:
...
protected:
	...
	bool m_bSBInit;		// define for the flag of the ScrollBar init
...
class CDXUTComboBox : public CDXUTControl
{
public:
    ...
    int    GetNumItems() { return m_Items.GetSize(); }
    DXUTComboBoxItem* GetItem( int index ) { return m_Items.GetAt( index ); }
...
public:
	...
	void SetButtonSize( int w, int h ){ iButtonWidth=w; iButtonHeight=h; }
	void SetTraceSize( int w, int h ){ iTraceWidth=w; iTraceHeight=h; }
...
// EditBox control
class CDXUTEditBox : public CDXUTButton
{
public:
...
// GroupBox control
class CDXUTGroupBox : public CDXUTButton
{
public:
    CDXUTGroupBox( CDXUTDialog *pDialog = NULL );
    virtual void UpdateRects();
    virtual void Render( float fElapsedTime );
    virtual BOOL ContainsPoint( POINT pt ) { return false; }
	void SetSpacing( int nSpacing ) { m_nSpacing = nSpacing; UpdateRects(); }
...
// Progress control
class CDXUTProgress : public CDXUTButton
{
public:
    CDXUTProgress( CDXUTDialog *pDialog = NULL );
    virtual void UpdateRects();
	virtual void Render( float fElapsedTime );
	...
	void SetHorizontal( bool Horizontal ){ m_bIsHorizontal = Horizontal; }
	int IsHorizontal(){ return m_bIsHorizontal; }
