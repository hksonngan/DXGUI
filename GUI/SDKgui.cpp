...
{   
	...
	assert( m_pManager->GetD3D9Device());
...
//--------------------------------------------------------------------------------------
HRESULT CDXUTDialog::AddCheckBox( int ID, LPCWSTR strText, int x, int y, int height, bool bChecked, UINT nHotkey, bool bIsDefault, CDXUTCheckBox** ppCreated )
{
    HRESULT hr = S_OK;
    ...
    pCheckBox->SetSize( height, height );
...
//--------------------------------------------------------------------------------------
HRESULT CDXUTDialog::AddRadioButton( int ID, UINT nButtonGroup, LPCWSTR strText, int x, int y, int height, bool bChecked, UINT nHotkey, bool bIsDefault, CDXUTRadioButton** ppCreated )
{
    HRESULT hr = S_OK;
    ...
    pRadioButton->SetSize( height, height );
...
//--------------------------------------------------------------------------------------
HRESULT CDXUTDialog::AddSlider( int ID, int x, int y, int width, int height, int bWidth, int tHeight, int min, int max, int value, bool bIsDefault, CDXUTSlider** ppCreated )
{
    HRESULT hr = S_OK;
	...
	pSlider->SetButtonSize( bWidth, height );
	pSlider->SetTraceSize( width, tHeight );
    pSlider->UpdateRects();
...
//--------------------------------------------------------------------------------------
HRESULT CDXUTDialog::AddGroupBox( int ID, LPCWSTR strText, int x, int y, int width, int height, bool bIsDefault, CDXUTGroupBox** ppCreated )
{
    HRESULT hr = S_OK;

    CDXUTGroupBox* pGroupBox = new CDXUTGroupBox( this );
    ...
    hr = AddControl( pGroupBox );
    if( FAILED(hr) )
        return hr;

    // Set the ID and list index
    pGroupBox->SetID( ID ); 
    pGroupBox->SetText(  strText );
    pGroupBox->SetLocation( x, y );
    pGroupBox->SetSize( width, height );
    pGroupBox->m_bIsDefault = bIsDefault;
...
//--------------------------------------------------------------------------------------
HRESULT CDXUTDialog::AddProgress( int ID, int x, int y, int width, int height,int value, int max, bool bIsHorizontal, bool bIsDefault, CDXUTProgress** ppCreated )
{
    HRESULT hr = S_OK;

    CDXUTProgress* pProgress = new CDXUTProgress( this );
    ...
    hr = AddControl( pProgress );
    if( FAILED(hr) )
        return hr;

    // Set the ID and list index
    pProgress->SetID( ID ); 
    pProgress->SetLocation( x, y );
	pProgress->SetMax( max );
	pProgress->SetValue( value );
    pProgress->SetSize( width, height );
	pProgress->SetHorizontal( bIsHorizontal );
    pProgress->m_bIsDefault = bIsDefault;
	return S_OK;
}
...
{
    RECT rcScreen = *pRect;
    RECTOffset( &rcScreen, m_x, m_y );

    // If caption is enabled, offset the Y position by its height.
    if( m_bCaption )
        RECTOffset( &rcScreen, 0, m_nCaptionHeight );
...
HRESULT CDXUTDialog::DrawSprite9( CDXUTElement* pElement, RECT* prcDest )
{
	...
	SPVector vec,vPos;
	RECTDiv((float *)&vec,&rcScreen,&rcTexture);
	RECTPointToVector((float *)&vPos,&rcScreen);
	
    SPMatrix matTransform;
	matTransform.ScaleMatrix( vec.x, vec.y, vec.z );

	m_pManager->m_pSprite->SetTransform( (D3DXMATRIX *)&matTransform );
    
	vPos.Scaling(vec);

    return m_pManager->m_pSprite->Draw( pTextureNode->pTexture9, &rcTexture, NULL, (D3DXVECTOR3 *)&vPos, pElement->TextureColor.Current );
}
...
HRESULT CDXUTDialog::DrawSprite10( CDXUTElement* pElement, RECT* prcDest, float fDepth )
{
    ...
    RECT rcScreen = *prcDest;
    RECTOffset( &rcScreen, m_x, m_y );
    RECTLefeTopMoveCenter( &rcScreen );
...
//--------------------------------------------------------------------------------------
void CDXUTDialog::ImageRect(DXUT_CONTROL_TYPE id, CDXUTElement &e, RECT &rc, RECT &sm, int iStat)
{
    RECT rcTexture;
	for (int i=0; i< 9 * e.iImageCount;i++)
	{
		if ( i < e.iImageCount )
			RECTSet( &rcTexture, rc.left, rc.top + rc.bottom * i, rc.left + sm.left, rc.top + rc.bottom * i + sm.top  );
		else if ( i < e.iImageCount * 2 )
			RECTSet( &rcTexture, rc.left + sm.left, rc.top + rc.bottom * ( i - e.iImageCount ), rc.left + rc.right - sm.right,  rc.top + rc.bottom * ( i - e.iImageCount ) + sm.top );
		else if ( i < e.iImageCount * 3 )
			RECTSet( &rcTexture, rc.left + rc.right - sm.right, rc.top + rc.bottom * ( i - e.iImageCount*2 ), rc.left + rc.right, rc.top + rc.bottom * ( i - e.iImageCount*2 ) + sm.top );
		else if ( i < e.iImageCount * 4 )
			RECTSet( &rcTexture, rc.left, rc.top + rc.bottom * ( i - e.iImageCount*3 ) + sm.top, rc.left + sm.left,  rc.top + rc.bottom * ( i - e.iImageCount*3 ) + rc.bottom - sm.bottom );
		else if ( i < e.iImageCount * 5 )
			RECTSet( &rcTexture, rc.left + sm.left, rc.top + rc.bottom * ( i - e.iImageCount*4 ) + sm.top, rc.left + rc.right - sm.right,  rc.top + rc.bottom * ( i - e.iImageCount*4 ) + rc.bottom - sm.bottom );
		else if ( i < e.iImageCount * 6 )
			RECTSet( &rcTexture, rc.left + rc.right - sm.right, rc.top + rc.bottom * ( i - e.iImageCount*5 ) + sm.top, rc.left + rc.right,  rc.top + rc.bottom * ( i - e.iImageCount*5 ) + rc.bottom - sm.bottom );
		else if ( i < e.iImageCount * 7 )
			RECTSet( &rcTexture, rc.left, rc.top + rc.bottom * ( i - e.iImageCount*6 ) + rc.bottom - sm.bottom, rc.left + sm.left,  rc.top + rc.bottom * ( i - e.iImageCount*6 ) + rc.bottom );
		else if ( i < e.iImageCount * 8 )
			RECTSet( &rcTexture, rc.left + sm.left, rc.top + rc.bottom * ( i - e.iImageCount*7 ) + rc.bottom - sm.bottom, rc.left + rc.right - sm.right,  rc.top + rc.bottom * ( i - e.iImageCount*7 ) + rc.bottom );
		else
			RECTSet( &rcTexture, rc.left + rc.right - sm.right, rc.top + rc.bottom * ( i - e.iImageCount*8 ) + rc.bottom - sm.bottom, rc.left + rc.right,  rc.top + rc.bottom * ( i - e.iImageCount*8 ) + rc.bottom );
		e.SetTexture( GUI_TEXTURE_DEFAULT, &rcTexture );
		SetDefaultElement( id, iStat+i, &e );
	}
}
//--------------------------------------------------------------------------------------
void CDXUTDialog::ImageRect(DXUT_CONTROL_TYPE id, CDXUTElement &e, RECT &rc, int iStat )
{
    RECT rcTexture;
	for (int i=0; i<e.iImageCount;i++)
	{
		RECTSet( &rcTexture, rc.left, rc.top + rc.bottom * i, rc.left + rc.right, rc.top + rc.bottom * ( i + 1 ) );
		e.SetTexture( GUI_TEXTURE_DEFAULT, &rcTexture );
		SetDefaultElement( id, iStat+i, &e );
	}
}
...
void CDXUTDialog::InitDefaultElements()
{
    ...
    RECTSet( &rcTexture, 17, 269, 241, 287 );
	...
	D3DCOLOR TextColor =  D3DCOLOR_ARGB( 255, 0, 0, 0 );
	D3DCOLOR BorderColor =  D3DCOLOR_ARGB( 255, 127, 157, 185 );
	D3DCOLOR FillColor =  D3DCOLOR_ARGB( 255, 255, 255, 255 );
	Element.SetImageCount( 4 );
	Element.SetImageBgType( true );
	Element.SetBorder( BorderColor, 1 );
	Element.SetFillColor( FillColor );
	Element.SetFont( 0, TextColor );
    Element.FontColor.States[ DXUT_STATE_DISABLED ] = D3DCOLOR_ARGB( 200, 200, 200, 200 );
	
	RECTSet( &rc1, 192, 0, 20, 23 );
	RECTSet( &rc2, 8, 9, 8, 9 );
    ImageRect( DXUT_CONTROL_BUTTON, Element, rc1, rc2, ID_BUTTON );

    // CDXUTCheckBox
	Element.SetImageCount( 8 );
	Element.SetFont( 0, D3DCOLOR_ARGB(255, 255, 255, 255), DT_NOCLIP | DT_LEFT | DT_VCENTER );
    Element.FontColor.States[ DXUT_STATE_DISABLED ] = D3DCOLOR_ARGB( 200, 200, 200, 200 );
	RECTSet( &rc1, 0, 0, 25, 25 );
	ImageRect( DXUT_CONTROL_CHECKBOX, Element, rc1, ID_CHECKBOX_LARGE );
	RECTSet( &rc1, 64, 0, 16, 16 );
	ImageRect( DXUT_CONTROL_CHECKBOX, Element, rc1, ID_CHECKBOX_MIDDLE );
	RECTSet( &rc1, 112, 0, 13, 13 );
	ImageRect( DXUT_CONTROL_CHECKBOX, Element, rc1, ID_CHECKBOX_SMALL );

	// CDXUTRadioButton
	Element.SetImageCount( 8 );
    Element.SetFont( 0, D3DCOLOR_ARGB(255, 255, 255, 255), DT_NOCLIP | DT_LEFT | DT_VCENTER );
    Element.FontColor.States[ DXUT_STATE_DISABLED ] = D3DCOLOR_ARGB( 200, 200, 200, 200 );
	RECTSet( &rc1, 32, 0, 25, 25 );
	ImageRect( DXUT_CONTROL_RADIOBUTTON, Element, rc1, ID_CHECKBOX_LARGE );
	RECTSet( &rc1, 80, 0, 16, 16 );
	ImageRect( DXUT_CONTROL_RADIOBUTTON, Element, rc1, ID_CHECKBOX_MIDDLE );
	RECTSet( &rc1, 80, 128, 13, 13 );
	ImageRect( DXUT_CONTROL_RADIOBUTTON, Element, rc1, ID_CHECKBOX_SMALL );
  
    // CDXUTSlider - Button
	Element.SetImageCount( 4 );
	RECTSet( &rc1, 96, 0, 11, 21 );
	RECTSet( &rc2, 3, 4, 3, 4 );
	ImageRect( DXUT_CONTROL_SLIDER, Element, rc1, rc2, ID_SLIDER_BUTTON );

	// CDXUTSlider - Track
	Element.SetImageCount( 1 );
	RECTSet( &rc1, 96, 112, 5, 5 );
	RECTSet( &rc2, 2, 2, 2, 2 );
    ImageRect( DXUT_CONTROL_SLIDER, Element, rc1, rc2, ID_SLIDER_TRACK );

	// CDXUTComboBox - Text
	FillColor =  D3DCOLOR_ARGB( 255, 255, 255, 255 );
	Element.SetFillColor( FillColor );
    Element.SetFont( 0, D3DCOLOR_ARGB(255, 0, 0, 0), DT_LEFT | DT_VCENTER );
    Element.FontColor.States[ DXUT_STATE_FOCUS ] = D3DCOLOR_ARGB( 255, 255, 255, 255 );
	Element.FillColor.States[ DXUT_STATE_FOCUS ] = D3DCOLOR_ARGB( 255, 10, 36, 106 );

	// CDXUTComboBox - Box
	Element.SetBorder( D3DCOLOR_ARGB( 255, 127, 157, 185 ), 1 );
	Element.SetImageBgType( false );

	// CDXUTComboBox - Button
	Element.SetImageCount( 4 );
	RECTSet( &rc1, 128, 128, 15, 16 );
	RECTSet( &rc2, 3, 3, 3, 3 );
    ImageRect( DXUT_CONTROL_COMBOBOX, Element, rc1, rc2, ID_COMBO_BUTTON );

    // CDXUTComboBox - Arrow
	RECTSet( &rc1, 144, 160, 9, 7 );
	ImageRect( DXUT_CONTROL_COMBOBOX, Element, rc1, ID_COMBO_ARROW );
	
	// CDXUTComboBox - Box
	RECTSet( &rc1, 192, 0, 20, 23 );
	RECTSet( &rc2, 8, 9, 8, 9 );
	ImageRect( DXUT_CONTROL_COMBOBOX, Element, rc1, rc2, ID_COMBO_BOX );

    // CDXUTScrollBar - ThumbButton
	Element.SetImageCount( 4 );
	RECTSet( &rc1, 192, 128, 22, 15 );
	RECTSet( &rc2, 5, 5, 5, 5 );
    ImageRect( DXUT_CONTROL_SCROLLBAR, Element, rc1, rc2, ID_THUMB_BUTTON_HORIZONTAL );
	RECTSet( &rc1, 96, 128, 15, 22 );
	RECTSet( &rc2, 5, 5, 5, 5 );
    ImageRect( DXUT_CONTROL_SCROLLBAR, Element, rc1, rc2, ID_THUMB_BUTTON_VERTICAL );
    
	// CDXUTScrollBar - ArrowButton
	RECTSet( &rc1, 224, 0, 17, 17 );
	RECTSet( &rc2, 5, 5, 5, 5 );
    ImageRect( DXUT_CONTROL_SCROLLBAR, Element, rc1, rc2, ID_ARROW_BUTTON_TOP );   
	RECTSet( &rc1, 224, 0, 17, 17 );
	RECTSet( &rc2, 5, 5, 5, 5 );
    ImageRect( DXUT_CONTROL_SCROLLBAR, Element, rc1, rc2, ID_ARROW_BUTTON_BOTTOM );   
	RECTSet( &rc1, 224, 0, 17, 17 );
	RECTSet( &rc2, 5, 5, 5, 5 );
    ImageRect( DXUT_CONTROL_SCROLLBAR, Element, rc1, rc2, ID_ARROW_BUTTON_LEFT );   
	RECTSet( &rc1, 224, 0, 17, 17 );
	RECTSet( &rc2, 5, 5, 5, 5 );
    ImageRect( DXUT_CONTROL_SCROLLBAR, Element, rc1, rc2, ID_ARROW_BUTTON_RIGHT );
	
	// CDXUTScrollBar - Track
	RECTSet( &rc1, 160, 0, 17, 17 );
    ImageRect( DXUT_CONTROL_SCROLLBAR, Element, rc1, ID_TRACK_TOP );
	RECTSet( &rc1, 160, 0, 17, 17 );
    ImageRect( DXUT_CONTROL_SCROLLBAR, Element, rc1, ID_TRACK_BOTTOM );
	RECTSet( &rc1, 160, 80, 17, 17 );
    ImageRect( DXUT_CONTROL_SCROLLBAR, Element, rc1, ID_TRACK_LEFT );
	RECTSet( &rc1, 160, 80, 17, 17 );
    ImageRect( DXUT_CONTROL_SCROLLBAR, Element, rc1, ID_TRACK_RIGHT );

	// CDXUTScrollBar - Arrow
	RECTSet( &rc1, 128, 0, 7, 7 );
    ImageRect( DXUT_CONTROL_SCROLLBAR, Element, rc1, ID_ARROW_SMALL_TOP );
	RECTSet( &rc1, 128, 28, 7, 7 );
    ImageRect( DXUT_CONTROL_SCROLLBAR, Element, rc1, ID_ARROW_SMALL_BOTTOM );
	RECTSet( &rc1, 128, 56, 7, 7 );
    ImageRect( DXUT_CONTROL_SCROLLBAR, Element, rc1, ID_ARROW_SMALL_LEFT );
	RECTSet( &rc1, 128, 84, 7, 7 );
    ImageRect( DXUT_CONTROL_SCROLLBAR, Element, rc1, ID_ARROW_SMALL_RIGHT );
	RECTSet( &rc1, 144, 0, 9, 9 );
    ImageRect( DXUT_CONTROL_SCROLLBAR, Element, rc1, ID_ARROW_LARGE_TOP );
	RECTSet( &rc1, 144, 36, 9, 9 );
    ImageRect( DXUT_CONTROL_SCROLLBAR, Element, rc1, ID_ARROW_LARGE_BOTTOM );
	RECTSet( &rc1, 144, 72, 9, 9 );
    ImageRect( DXUT_CONTROL_SCROLLBAR, Element, rc1, ID_ARROW_LARGE_LEFT );
	RECTSet( &rc1, 144, 108, 9, 9 );
    ImageRect( DXUT_CONTROL_SCROLLBAR, Element, rc1, ID_ARROW_LARGE_RIGHT );

	// CDXUTScrollBar - Gripper
	RECTSet( &rc1, 96, 224, 8, 8 );
    ImageRect( DXUT_CONTROL_SCROLLBAR, Element, rc1, ID_GRIPPER_HORIZONTAL );
	RECTSet( &rc1, 112, 224, 8, 8 );
    ImageRect( DXUT_CONTROL_SCROLLBAR, Element, rc1, ID_GRIPPER_VERTICAL );
	...
	Element.SetBorder( D3DCOLOR_ARGB( 255, 127, 157, 185 ), 1 );
	Element.SetImageBgType( false );
	SetDefaultElement( DXUT_CONTROL_EDITBOX, ID_EDITBOX, &Element );
	RECTSet( &rc1, 192, 0, 20, 23 );
	RECTSet( &rc2, 8, 9, 8, 9 );
	ImageRect( DXUT_CONTROL_EDITBOX, Element, rc1, rc2, ID_EDITBOX );
	...
	Element.SetBorder( D3DCOLOR_ARGB( 255, 127, 157, 185 ), 1 );
	Element.SetImageBgType( false );
	SetDefaultElement( DXUT_CONTROL_LISTBOX, ID_LISTBOX, &Element );
	RECTSet( &rc1, 192, 0, 20, 23 );
	RECTSet( &rc2, 8, 9, 8, 9 );
	ImageRect( DXUT_CONTROL_LISTBOX, Element, rc1, rc2, ID_LISTBOX );

	// CDXUTGroupBox
	Element.SetImageCount( 1 );
	Element.SetFont( 0, D3DCOLOR_ARGB(255, 255, 255, 255), DT_NOCLIP | DT_LEFT | DT_TOP );
	RECTSet( &rc1, 192, 224, 22, 20 );
	RECTSet( &rc2, 8, 8, 7, 9 );
    ImageRect( DXUT_CONTROL_GROUPBOX, Element, rc1, rc2, ID_GROUPBOX );

	// CDXUTProgress
	Element.SetImageCount( 1 );
	RECTSet( &rc1, 128, 192, 9, 19 );
	RECTSet( &rc2, 4, 4, 3, 3 );
    ImageRect( DXUT_CONTROL_PROGRESS, Element, rc1, rc2, ID_PROGRESS_BOX );
	RECTSet( &rc1, 144, 192, 10, 12 );
	ImageRect( DXUT_CONTROL_PROGRESS, Element, rc1, ID_PROGRESS_CHUNK );
}
//--------------------------------------------------------------------------------------
...
//--------------------------------------------------------------------------------------
void CDXUTControl::DrawBorder( RECT &rc, int iState, float fElapsedTime )
{
	RECT rcWindow=rc;
    CDXUTElement* pElement = m_Elements.GetAt( 0 );
	m_pDialog->DrawRect( &rcWindow,pElement->BorderColor.States[iState] );
	RECTInflate( &rcWindow, pElement->iBorder, pElement->iBorder, - pElement->iBorder, - pElement->iBorder );	
	m_pDialog->DrawRect( &rcWindow,pElement->FillColor.States[iState] );
}
//--------------------------------------------------------------------------------------
void CDXUTControl::DrawResize( RECT &rc, int iState, int ID, float fElapsedTime )
{
	CDXUTElement* pElement = m_Elements.GetAt( ID + iState);
	pElement->TextureColor.Blend( iState, fElapsedTime, 0.0f );
	m_pDialog->DrawSprite( pElement, &rc, DXUT_NEAR_BUTTON_DEPTH );
}
//--------------------------------------------------------------------------------------
void CDXUTControl::DrawTile( RECT &rc, int iState, int ID, float fElapsedTime, int nCount, bool bIsHorizontal)
{
	CDXUTElement* pElement = m_Elements.GetAt( ID + iState);
	pElement->TextureColor.Blend( iState, fElapsedTime, 0.0f );
	RECT rcTile=rc;	
	SIZE size,sizeRc;
	RECTSize(&size,&pElement->rcTexture);
	RECTSize(&sizeRc,&rc);
	if ( bIsHorizontal )
	{
		RECTReSize ( &rcTile, size.cx, sizeRc.cy);
		for ( int i=0; i< nCount; i++ )
		{			
			m_pDialog->DrawSprite( pElement, &rcTile, DXUT_NEAR_BUTTON_DEPTH );
			RECTOffset ( &rcTile, size.cx, 0 );
		}
		rcTile.right = rc.right;
		m_pDialog->DrawSprite( pElement, &rcTile, DXUT_NEAR_BUTTON_DEPTH );
	}
	else
	{
		RECTReSize ( &rcTile, sizeRc.cx, size.cy);
		for ( int i=0; i< nCount; i++ )
		{			
			m_pDialog->DrawSprite( pElement, &rcTile, DXUT_NEAR_BUTTON_DEPTH );
			RECTOffset ( &rcTile, 0, size.cy );
		}	
		rcTile.bottom = rc.bottom;
		m_pDialog->DrawSprite( pElement, &rcTile, DXUT_NEAR_BUTTON_DEPTH );
	}
}
//--------------------------------------------------------------------------------------
void CDXUTControl::DrawStretch( int iState,int ID, RECT* rc, float fElapsedTime, int n )
{
	CDXUTElement* pElement = m_Elements.GetAt( ID );
	for ( int i = 0;i < 9;i++ )
		DrawResize( rc[i+n], iState, ID + pElement->iImageCount * i, fElapsedTime );
}
//--------------------------------------------------------------------------------------
void CDXUTControl::SetStretch( int ID, RECT &rc, RECT* rcWindow,int i )
{
	CDXUTElement* pElementFirst = m_Elements.GetAt( ID );
	CDXUTElement* pElementLast = m_Elements.GetAt( ID + 8 );
	SIZE fSize,lSize;
	RECTSize (&fSize,&pElementFirst->rcTexture);
	RECTSize (&lSize,&pElementLast->rcTexture);

	RECTSet( &rcWindow[i], rc.left, rc.top, rc.left + fSize.cx, rc.top + fSize.cy );
	RECTSet( &rcWindow[i+1], rc.left + fSize.cx, rc.top, rc.right - lSize.cx , rc.top + fSize.cy );
	RECTSet( &rcWindow[i+2], rc.right - lSize.cx, rc.top, rc.right , rc.top + fSize.cy );

	RECTSet( &rcWindow[i+3], rc.left, rc.top + fSize.cy, rc.left + fSize.cx, rc.bottom - lSize.cy );
	RECTSet( &rcWindow[i+4], rc.left + fSize.cx, rc.top + fSize.cy, rc.right - lSize.cx , rc.bottom - lSize.cy  );
	RECTSet( &rcWindow[i+5], rc.right - lSize.cx, rc.top + fSize.cy, rc.right , rc.bottom - lSize.cy );

	RECTSet( &rcWindow[i+6], rc.left,  rc.bottom - lSize.cy, rc.left + fSize.cx, rc.bottom );
	RECTSet( &rcWindow[i+7], rc.left + fSize.cx,  rc.bottom - lSize.cy, rc.right - lSize.cx , rc.bottom );
	RECTSet( &rcWindow[i+8], rc.right - lSize.cx,  rc.bottom - lSize.cy, rc.right , rc.bottom );
}
//--------------------------------------------------------------------------------------
...
{
    RECTSet( &m_rcBoundingBox, m_x, m_y, m_x + m_width, m_y + m_height );
}
...
//--------------------------------------------------------------------------------------
void CDXUTButton::UpdateRects()
{
    CDXUTStatic::UpdateRects();
	SetStretch( ID_BUTTON,m_rcBoundingBox,m_rcStretch );
}
...
void CDXUTButton::Render( float fElapsedTime )
{
    ...
    CDXUTElement* pElement = m_Elements.GetAt( 0 );
	if ( !pElement->isImageBgType() )
	{
		DrawBorder( m_rcBoundingBox, iState, fElapsedTime );
		pElement->FontColor.Blend( iState, fElapsedTime);
		m_pDialog->DrawText( m_strText, pElement, &m_rcBoundingBox );
		return ;
	}
	DrawStretch( iState, ID_BUTTON, m_rcStretch, fElapsedTime );
	pElement->FontColor.Blend( iState, fElapsedTime,0.0f );
	m_pDialog->DrawText( m_strText, pElement, &m_rcBoundingBox );
}
//--------------------------------------------------------------------------------------
...
{ 
    return ( RECTPointIn( &m_rcButton, pt ) );
}
...
void CDXUTCheckBox::UpdateRects()
{
	...
	SIZE size;
	RECTSize(&size,&m_rcBoundingBox);
	RECTReSize(&m_rcButton,size.cy,size.cy);
	
    m_rcText = m_rcBoundingBox;
    m_rcText.left += (int) ( 1.25f * size.cy );//0.25f is the space between the checkbox and the text 
}
...
void CDXUTCheckBox::Render( float fElapsedTime )
{
	...
	{
		if (!m_bEnabled)
			iState = DXUT_STATE_CHECKED_DISABLE;
		else if (m_bPressed)
			iState = DXUT_STATE_CHECKED_PRESSED;
		else if (m_bMouseOver)
			iState = DXUT_STATE_CHECKED_MOUSEOVER;
		else 
			iState = DXUT_STATE_CHECKED;			
	}
	else
	{
		if (!m_bEnabled)
	...
	if (m_height <= GUI_SIZE_SMALL)
		DrawResize( m_rcButton, iState, ID_CHECKBOX_SMALL, fElapsedTime );
	else if (m_height <= GUI_SIZE_MIDDLE)
		DrawResize( m_rcButton, iState, ID_CHECKBOX_MIDDLE, fElapsedTime );
	else
		DrawResize( m_rcButton, iState, ID_CHECKBOX_LARGE, fElapsedTime );
}
...
void CDXUTComboBox::UpdateRects()
{
	...
	if ( GetElement( 0 )->isImageBgType() )
		SetStretch( ID_COMBO_BOX,m_rcBoundingBox,m_rcStretch );

	CDXUTElement* pElement = m_Elements.GetAt( ID_COMBO_BUTTON );
    m_rcButton = m_rcBoundingBox;
	SIZE size;
	RECTSize(&size,&m_rcButton);
	RECTOffset( &m_rcButton, m_width - size.cy  + pElement->iBorder,  pElement->iBorder );
	RECTReSize( &m_rcButton, size.cy - 2*pElement->iBorder, size.cy - 2*pElement->iBorder);

	SetStretch( ID_COMBO_BUTTON,m_rcButton,m_rcStretch,9 );

	pElement = m_Elements.GetAt( ID_COMBO_ARROW );
	m_rcArrow = m_rcButton;
	RECTLefeTopMoveCenter( &m_rcArrow,&m_rcButton );
	RECTCenterMoveLeftTop( &m_rcArrow,&pElement->rcTexture );
	RECTOffset( &m_rcArrow, 0, -1 );
	RECTReSize( &m_rcArrow, &pElement->rcTexture );

    m_rcText = m_rcBoundingBox;
	RECTOffset( &m_rcText, m_nSpacing, 0 );
	RECTReSize( &m_rcText, m_width - size.cy , size.cy );
    ...
    {
        m_ScrollBar.SetPageSize( m_nDropHeight / pFontNode->nHeight );
...
bool CDXUTComboBox::HandleMouse( UINT uMsg, POINT pt, WPARAM wParam, LPARAM lParam )
{
    ...
    if( m_bOpened && m_bEnabled && m_ScrollBar.HandleMouse( uMsg, pt, wParam, lParam ) )
        return true;
    switch( uMsg )
    {
        case WM_MOUSEMOVE:
        {
            if( m_bOpened && RECTPointIn( &m_rcDropdown, pt ) )
            {
...
void CDXUTComboBox::Render( float fElapsedTime )
{
	...
	{
		// Border
		DrawBorder( m_rcDropdown, iState, fElapsedTime );
		// Scroll bar
		m_ScrollBar.SetLocation( m_rcDropdown.right - 1, m_rcDropdown.top + 1 );
		m_ScrollBar.SetSize( m_nSBWidth, RectHeight( m_rcDropdown ) - 1 );
    ...
    if( pFont )
    {
        ...
        for( int i = m_ScrollBar.GetTrackPos(); i < m_Items.GetSize(); i++ )
        {
            ...
            RECTSet( &pItem->rcActive, m_rcDropdownText.left, curY, m_rcDropdownText.right, curY + pFont->nHeight );
            curY += pFont->nHeight;
            ...
            if( m_bOpened )
            {
                ...
                {
                    RECT rc;
                    RECTSet( &rc, m_rcDropdown.left, pItem->rcActive.top-1, m_rcDropdown.right, pItem->rcActive.bottom+1 );
					m_pDialog->DrawRect(&rc, pElement->FillColor.States[DXUT_STATE_FOCUS]);
					pElement->FontColor.Current= pElement->FontColor.States[ DXUT_STATE_FOCUS ];
	...
	//draw border
	if ( pElement->isImageBgType() )
		DrawStretch( DXUT_STATE_NORMAL, ID_COMBO_BOX,m_rcStretch, fElapsedTime );
	else
		DrawBorder( m_rcBoundingBox, 0, fElapsedTime );
	//draw button
	DrawStretch( iState, ID_COMBO_BUTTON, m_rcStretch, fElapsedTime, 9 );
	DrawResize( m_rcArrow,iState, ID_COMBO_ARROW, fElapsedTime);
...
HRESULT CDXUTComboBox::AddItem( const WCHAR* strText, void* pData )
{
	...
	CDXUTElement* pElement = m_Elements.GetAt( 0 );
	DXUTFontNode* pFontNode = m_pDialog->GetManager()->GetFontNode( pElement->iFont );

	m_rcDropdown = m_rcText;
    RECTOffset( &m_rcDropdown, - m_nSpacing , RectHeight( m_rcText ) - pElement->iBorder );
	if ( GetNumItems() > m_ScrollBar.GetPageSize() )
		RECTReSize ( &m_rcDropdown, m_width - m_nSBWidth + pElement->iBorder , __min ( m_nDropHeight, pFontNode->nHeight * GetNumItems() ));
	else
		RECTReSize ( &m_rcDropdown, m_width, __min ( m_nDropHeight, pFontNode->nHeight * GetNumItems() ));
    m_rcDropdownText = m_rcDropdown;
	RECTOffset( &m_rcDropdownText, m_nSpacing , 0 );
...
//--------------------------------------------------------------------------------------
HRESULT CDXUTComboBox::SetSelectedByIndex( int index )
{
...
{ 
    return ( RECTPointIn( &m_rcBoundingBox, pt ) || 
             RECTPointIn( &m_rcButton, pt ) ); 
}
...
{
    CDXUTControl::UpdateRects();
	m_rcTrack = m_rcBoundingBox;
	m_rcTrack.top = m_rcTrack.top + ( RectHeight( m_rcTrack ) - iTraceHeight )/2;
	m_rcTrack.bottom = m_rcTrack.top + iTraceHeight;
	SetStretch( ID_SLIDER_TRACK,m_rcTrack,m_rcStretch );
    m_rcButton = m_rcBoundingBox;
    m_rcButton.right = m_rcButton.left + iButtonWidth;
    RECTOffset( &m_rcButton, -iButtonWidth/2, 0 );
	SIZE size;
	RECTSize(&size,&m_rcBoundingBox);
	m_nButtonX = (int) ( (m_nValue - m_nMin) * (float)size.cx / (m_nMax - m_nMin) );
    RECTOffset( &m_rcButton, m_nButtonX, 0 );
	SetStretch( ID_SLIDER_BUTTON,m_rcButton,m_rcStretch,9 );
}
...
{ 
	SIZE size;
	RECTSize(&size,&m_rcBoundingBox);
    float fValuePerPixel = (float)(m_nMax - m_nMin) / size.cx;
...
bool CDXUTSlider::HandleMouse( UINT uMsg, POINT pt, WPARAM wParam, LPARAM lParam )
{
    ...
    switch( uMsg )
    {
        ...
        case WM_LBUTTONDBLCLK:
        {
            ...
            if( RECTPointIn( &m_rcBoundingBox, pt ) )
            {
                m_nDragX = pt.x;
                m_nDragOffset = 0;               
                //m_bPressed = true;
...
void CDXUTSlider::Render( float fElapsedTime )
{
	...
	if (!m_bEnabled)
		...
		iState = DXUT_STATE_MOUSEOVER;
	DrawStretch( DXUT_STATE_NORMAL, ID_SLIDER_TRACK,m_rcStretch,fElapsedTime );
	DrawStretch( iState, ID_SLIDER_BUTTON, m_rcStretch, fElapsedTime, 9 );
}
//--------------------------------------------------------------------------------------
CDXUTScrollBar::CDXUTScrollBar( CDXUTDialog *pDialog, bool IsHorizontal)
{
	...
	m_bArrowMinPressed = false;
	m_bArrowMaxPressed = false;
	m_bTrackMinPressed = false;
	m_bTrackMaxPressed = false;
	m_bArrowMinMouseOver = false;
	m_bArrowMaxMouseOver = false;
	m_bTrackMinMouseOver = false;
	m_bTrackMaxMouseOver = false;

    RECTSet( &m_rcMinButton, 0, 0, 0, 0 );
    RECTSet( &m_rcMaxButton, 0, 0, 0, 0 );
    RECTSet( &m_rcThumb, 0, 0, 0, 0 );
    RECTSet( &m_rcMinArrow, 0, 0, 0, 0 );
    RECTSet( &m_rcMaxArrow, 0, 0, 0, 0 );
    RECTSet( &m_rcTrack, 0, 0, 0, 0 );
    RECTSet( &m_rcMinTrack, 0, 0, 0, 0 );
    RECTSet( &m_rcMaxTrack, 0, 0, 0, 0 );
    m_nPosition = 0;
    m_nPageSize = 1;
    m_nStart = 0;
    m_nEnd = 1;
...
void CDXUTScrollBar::UpdateRects()
{
	...
	m_rcMinButton = m_rcBoundingBox;
	SIZE size;
	RECTSize(&size,&m_rcBoundingBox);
	RECTReSize ( &m_rcMinButton, size.cx,  size.cx);
	SetStretch( ID_ARROW_BUTTON_TOP,m_rcMinButton,m_rcStretch,9 );
	m_rcMaxButton = m_rcMinButton;
    RECTOffset ( &m_rcMaxButton, 0, size.cy - size.cx);
	SetStretch( ID_ARROW_BUTTON_BOTTOM,m_rcMaxButton,m_rcStretch,18 );

	CDXUTElement* pElement;
	SIZE sizeButton;
	RECTSize(&sizeButton,&m_rcMinButton);
	if ( sizeButton.cx < GUI_SIZE_MIDDLE || sizeButton.cy < GUI_SIZE_MIDDLE )
		pElement = m_Elements.GetAt( ID_ARROW_SMALL_TOP );
	else
		pElement = m_Elements.GetAt( ID_ARROW_LARGE_TOP );
	m_rcMinArrow = m_rcMinButton;
	
	RECTLefeTopMoveCenter( &m_rcMinArrow,&m_rcMinButton );
	RECTCenterMoveLeftTop( &m_rcMinArrow,&pElement->rcTexture );

	RECTReSize ( &m_rcMinArrow, &pElement->rcTexture );
	m_rcMaxArrow = m_rcMaxButton;
	RECTLefeTopMoveCenter( &m_rcMaxArrow,&m_rcMaxButton );
	RECTCenterMoveLeftTop( &m_rcMaxArrow,&pElement->rcTexture );

	RECTReSize ( &m_rcMaxArrow, &pElement->rcTexture );

	RECTSet( &m_rcTrack, m_rcMinButton.left, m_rcMinButton.bottom,
                         m_rcMaxButton.right, m_rcMaxButton.top );
    m_rcThumb.left = m_rcMinButton.left;
    m_rcThumb.right = m_rcMinButton.right;
...
void CDXUTScrollBar::UpdateThumbRect()
{
	...
	SetStretch( ID_THUMB_BUTTON_VERTICAL,m_rcThumb,m_rcStretch );
}
...
bool CDXUTScrollBar::HandleMouse( UINT uMsg, POINT pt, WPARAM wParam, LPARAM lParam )
{
    ...
    switch( uMsg )
    {
        ...
        case WM_LBUTTONDBLCLK:
        {
            ...
            if( RECTPointIn( &m_rcMinButton, pt ) )
            {
				m_bArrowMinPressed = true;
                SetCapture( DXUTGetHWND() );
            ...
            if( RECTPointIn( &m_rcMaxButton, pt ) )
            {
				m_bArrowMaxPressed = true;
				SetCapture( DXUTGetHWND() );
        ...
        {
            if( RECTPointIn( &m_rcMinButton, pt ) )
				m_bArrowMinMouseOver = true;
			else
				m_bArrowMinMouseOver = false;
            if( RECTPointIn( &m_rcMaxButton, pt ) )
				m_bArrowMaxMouseOver = true;
			else
				m_bArrowMaxMouseOver = false;
            if( RECTPointIn( &m_rcMinTrack, pt ) )
				m_bTrackMinPressed=true;
			else
				m_bTrackMinPressed=false;
            if( RECTPointIn( &m_rcMaxTrack, pt ) )
				m_bTrackMaxPressed=true;
			else
				m_bTrackMaxPressed=false;
            if( RECTPointIn( &m_rcThumb, pt ) )
			...
			if( m_bDrag )
            {
                ...
                if( m_rcThumb.top < m_rcTrack.top )
                    RECTOffset( &m_rcThumb, 0, m_rcTrack.top - m_rcThumb.top );
                else
                if( m_rcThumb.bottom > m_rcTrack.bottom )
                    RECTOffset( &m_rcThumb, 0, m_rcTrack.bottom - m_rcThumb.bottom );
...
{
    DXUT_CONTROL_STATE iState = DXUT_STATE_NORMAL;
	DXUT_CONTROL_STATE iArrowMin = DXUT_STATE_NORMAL;
	DXUT_CONTROL_STATE iArrowMax = DXUT_STATE_NORMAL;
	DXUT_CONTROL_STATE iTrackMin = DXUT_STATE_NORMAL;
	DXUT_CONTROL_STATE iTrackMax = DXUT_STATE_NORMAL;

    if(!m_bVisible || !m_bShowThumb )
		return;

	if( !m_bEnabled )
	{
		iArrowMin = DXUT_STATE_DISABLED;
		iArrowMax = DXUT_STATE_DISABLED;
		iTrackMin = DXUT_STATE_DISABLED;
		iTrackMax = DXUT_STATE_DISABLED;
		iState = DXUT_STATE_DISABLED;
	}
	else
	{
		if (m_bDrag)
			...
			iState = DXUT_STATE_MOUSEOVER;
		if (m_bArrowMinPressed)
			iArrowMin = DXUT_STATE_PRESSED;
		else if (m_bArrowMinMouseOver)
			iArrowMin = DXUT_STATE_MOUSEOVER;
		if (m_bArrowMaxPressed)
			iArrowMax = DXUT_STATE_PRESSED;
		else if (m_bArrowMaxMouseOver)
			iArrowMax = DXUT_STATE_MOUSEOVER;
		if (m_bTrackMinPressed)
			iTrackMin = DXUT_STATE_PRESSED;
		else if (m_bTrackMinMouseOver)
			iTrackMin = DXUT_STATE_MOUSEOVER;
		if (m_bTrackMaxPressed)
			iTrackMax = DXUT_STATE_PRESSED;
		else if (m_bTrackMaxMouseOver)
			iTrackMax = DXUT_STATE_MOUSEOVER;
    ...
    if( m_Arrow != CLEAR )
    {
        ...
        if( RECTPointIn( &m_rcMinButton, m_LastMouse ) )
        {
            switch( m_Arrow )
            {
                case CLICKED_UP:
        ...
        if( RECTPointIn( &m_rcMaxButton, m_LastMouse ) )
        {
            switch( m_Arrow )
            {
                case CLICKED_DOWN:
	...
	//draw arrow button
	DrawStretch( iArrowMin, ID_ARROW_BUTTON_TOP, m_rcStretch,fElapsedTime,9 );
	DrawStretch( iArrowMax, ID_ARROW_BUTTON_BOTTOM, m_rcStretch,fElapsedTime,18 );
	//draw arrow
	SIZE size;
	RECTSize(&size,&m_rcMinButton);
	if (  size.cx < GUI_SIZE_MIDDLE || size.cy < GUI_SIZE_MIDDLE )
	{
		DrawResize( m_rcMinArrow,iState, ID_ARROW_SMALL_TOP, fElapsedTime);
		DrawResize( m_rcMaxArrow,iState, ID_ARROW_SMALL_BOTTOM, fElapsedTime);
	}
	else
	{
		DrawResize( m_rcMinArrow,iState, ID_ARROW_LARGE_TOP, fElapsedTime);
		DrawResize( m_rcMaxArrow,iState, ID_ARROW_LARGE_BOTTOM, fElapsedTime);
	}
	if ( m_bEnabled)
	{
		//draw Thumb button
		DrawStretch( iState, ID_THUMB_BUTTON_VERTICAL,m_rcStretch, fElapsedTime );
		//draw Gripper
		CDXUTElement* pElement = m_Elements.GetAt( ID_GRIPPER_VERTICAL );
		RECT rcGripper = m_rcThumb;
		RECTLefeTopMoveCenter( &rcGripper,&m_rcThumb );
		RECTCenterMoveLeftTop( &rcGripper,&pElement->rcTexture );
		RECTReSize ( &rcGripper, &pElement->rcTexture );
		SIZE size;
		RECTSize( &size,&m_rcThumb );
		if ( size.cy > SCROLLBAR_MINTHUMBSIZE )
			DrawResize( rcGripper, iState, ID_GRIPPER_VERTICAL, fElapsedTime);
		//draw Track
		RECTSet( &m_rcMinTrack, m_rcTrack.left, m_rcTrack.top, m_rcTrack.right, m_rcThumb.top );
		RECTSet( &m_rcMaxTrack, m_rcTrack.left, m_rcThumb.bottom, m_rcTrack.right, m_rcTrack.bottom );
		DrawResize( m_rcMinTrack, iTrackMin, ID_TRACK_TOP, fElapsedTime);
		DrawResize( m_rcMaxTrack, iTrackMax, ID_TRACK_BOTTOM, fElapsedTime);
	}
	else
	{
		DrawResize( m_rcTrack, iTrackMin, ID_TRACK_TOP, fElapsedTime);
	}
}
...
void CDXUTListBox::UpdateRects()
{
    CDXUTControl::UpdateRects();
	if (GetElement(0)->isImageBgType())
		SetStretch(  ID_LISTBOX,m_rcBoundingBox,m_rcStretch );
    m_rcSelection = m_rcBoundingBox;
    m_rcSelection.right -= m_nSBWidth;
	RECTInflate( &m_rcSelection, m_nBorder,m_nBorder, -m_nBorder, -m_nBorder );
    m_rcText = m_rcSelection;
	RECTInflate( &m_rcText, m_nMargin,m_nMargin, 0, 0 );
...
HRESULT CDXUTListBox::AddItem( const WCHAR *wszText, void *pData )
{
    ...
    RECTSet( &pNewItem->rcActive, 0, 0, 0, 0 );
...
bool CDXUTListBox::HandleMouse( UINT uMsg, POINT pt, WPARAM wParam, LPARAM lParam )
{
    ...
    switch( uMsg )
    {
        ...
        case WM_LBUTTONDBLCLK:
            ...
            if( m_Items.GetSize() > 0 && RECTPointIn( &m_rcSelection, pt ) )
            {
...
void CDXUTListBox::Render( float fElapsedTime )
{
	...
	if (GetElement(0)->isImageBgType())
		DrawStretch( DXUT_STATE_NORMAL,DXUT_STATE_NORMAL, m_rcStretch, fElapsedTime );
	else
		DrawBorder( m_rcBoundingBox,DXUT_STATE_NORMAL, fElapsedTime );
    // Render the text
    if( m_Items.GetSize() > 0 )
    {
...
void CDXUTEditBox::PlaceCaret( int nCP )
{
	...
	if( nX2 > nX1st + size.cx )
    {
        // Compute the X of the new left-most pixel
        int nXNewLeft = nX2 - size.cx;
...
void CDXUTEditBox::UpdateRects()
{
	...
	RECTInflate( &m_rcText, m_nBorder,m_nBorder, -m_nBorder, -m_nBorder );
    // Update the render rectangles
	if ( GetElement( 0 )->isImageBgType() )
		SetStretch( ID_EDITBOX,m_rcBoundingBox,m_rcStretch );
    // Inflate further by m_nSpacing
	RECTInflate( &m_rcText, m_nSpacing,m_nSpacing, -m_nSpacing, -m_nSpacing );
}
...
void CDXUTEditBox::Render( float fElapsedTime )
{
	...
	if ( pElement->isImageBgType() )
		DrawStretch( DXUT_STATE_NORMAL, ID_EDITBOX, m_rcStretch, fElapsedTime );
	else
		DrawBorder( m_rcBoundingBox, ID_EDITBOX, fElapsedTime );
    ...
    if( m_nCaret != m_nSelStart )
    {
        ...
        RECTSet( &rcSelection, nSelLeftX, m_rcText.top, nSelRightX, m_rcText.bottom );
        RECTOffset( &rcSelection, m_rcText.left - nXFirst, 0 );
		RECTIntersect ( &rcSelection, &m_rcText, &rcSelection );
...
//--------------------------------------------------------------------------------------
CDXUTGroupBox::CDXUTGroupBox( CDXUTDialog *pDialog )
{
    m_Type = DXUT_CONTROL_GROUPBOX;
    m_pDialog = pDialog;
	m_nSpacing = 10;
}
//--------------------------------------------------------------------------------------
void CDXUTGroupBox::UpdateRects()
{
    CDXUTControl::UpdateRects();

    DXUTFontNode* pFontNode = m_pDialog->GetManager()->GetFontNode( m_Elements.GetAt( ID_GROUPBOX )->iFont );

	m_rcBox = m_rcBoundingBox;
	RECTOffset ( &m_rcBox, 0, pFontNode->nHeight/2 );
	SIZE size;
	RECTSize( &size,&m_rcBox );
	RECTReSize ( &m_rcBox, size.cx, size.cy - pFontNode->nHeight/2 );

	SetStretch( ID_GROUPBOX, m_rcBox,m_rcStretch );
...
//--------------------------------------------------------------------------------------
void CDXUTGroupBox::Render( float fElapsedTime )
{
	DrawStretch( DXUT_STATE_NORMAL, ID_GROUPBOX, m_rcStretch, fElapsedTime );
    CDXUTElement* pElement = m_Elements.GetAt( ID_GROUPBOX );
    pElement->FontColor.Blend( DXUT_STATE_NORMAL, fElapsedTime );
    m_pDialog->DrawText( m_strText, pElement, &m_rcText, true );
}
...
//--------------------------------------------------------------------------------------
CDXUTProgress::CDXUTProgress( CDXUTDialog *pDialog )
{
    m_Type = DXUT_CONTROL_PROGRESS;
    m_pDialog = pDialog;
}
//--------------------------------------------------------------------------------------
void CDXUTProgress::UpdateRects()
{
    CDXUTControl::UpdateRects();
	SetStretch( ID_PROGRESS_BOX,m_rcBoundingBox,m_rcStretch );
	
	m_rcChunk=m_rcStretch[4];
	SIZE sBox,sTex;
	RECTSize(&sBox,&m_rcChunk);
	int width=m_nValue * ( sBox.cx )/m_nMax;
	RECTReSize ( &m_rcChunk, width, sBox.cy );
	
	CDXUTElement* pElement = m_Elements.GetAt( ID_PROGRESS_CHUNK );
	RECTSize(&sTex,&pElement->rcTexture);
	if ( m_bIsHorizontal )
		m_nCount=width/sTex.cx;
	else
		m_nCount=sBox.cy/sTex.cy;
}

//--------------------------------------------------------------------------------------
void CDXUTProgress::Render( float fElapsedTime )
{
	DrawStretch( DXUT_STATE_NORMAL, ID_PROGRESS_BOX, m_rcStretch, fElapsedTime );
	DrawTile( m_rcChunk, DXUT_STATE_NORMAL, ID_PROGRESS_CHUNK, fElapsedTime, m_nCount );
}
//--------------------------------------------------------------------------------------
void CDXUTProgress::Increment ( int iValue )
{
	if ( iValue < m_nMax )
		m_nValue += iValue;
	UpdateRects();
